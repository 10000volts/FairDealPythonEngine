...
class E2(EffNextTurnMixin):
    ...
    def execute(self):
        ...
        if self.scr_arg[2] < 2:
            ...
            if self.scr_arg[0].DEF.value > 8000:
                self.scr_arg[0].DEF.gain(-8000, False, self)
            else:
...
class E1(EffCommonStrategy):
    ...
    def execute(self):
        ...
        tp = TimePoint(ETimePoint.TRY_HEAL, self, [self.host, p.leader, 10000, 1])
        self.game.enter_time_point(tp)
        if tp.args[-1]:
            self.game.heal(self.host, p.leader, 10000, self)
